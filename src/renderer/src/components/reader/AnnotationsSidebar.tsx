...
export function AnnotationsSidebar({
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className="fixed right-0 top-0 bottom-0 w-80 bg-white z-40 shadow-xl flex flex-col border-l border-gray-200">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto">
          {filteredHighlights.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center px-6">
              <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="text-gray-300 mb-3">
                <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
              </svg>
              <p className="text-sm text-gray-400">No annotations yet</p>
              <p className="text-xs text-gray-300 mt-1">Select text to highlight</p>
            </div>
          ) : (
        ...
        {/* Stats footer */}
        <div className="px-4 py-2 border-t border-gray-200 text-xs text-gray-400 shrink-0">
  ...
  return (
    <div className="px-4 py-3 hover:bg-gray-50 transition-colors">
      ...
      <div className="flex gap-2">
        ...
        <button
          onClick={onNavigate}
          className="text-sm text-gray-700 text-left leading-relaxed line-clamp-3 hover:text-blue-600 transition-colors cursor-pointer"
      ...
      <div className="flex items-center gap-1 mt-2 ml-3">
        <button
          onClick={onAddNote}
          className="text-[11px] text-gray-400 hover:text-blue-500 transition-colors px-1.5 py-0.5 rounded hover:bg-blue-50"
          title="Add note"
        >
          + Note
        </button>
        ...
        <button
          ...
          className="text-[11px] text-gray-400 hover:text-red-500 transition-colors px-1.5 py-0.5 rounded hover:bg-red-50 ml-auto"
          ...
          {HIGHLIGHT_COLORS.map((c) => (
            <button
              ...
              className={`w-5 h-5 rounded-full border-2 transition-all ${
                highlight.color === c.value ? 'border-gray-500 scale-110' : 'border-transparent hover:border-gray-300'
              }`}
      ...
      {notes.map((note) => {
        ...
        return (
          <div key={note.id} className="ml-3 mt-2 pl-3 border-l-2 border-gray-200">
            {editingNoteId === note.id ? (
              <div className="space-y-1">
                <textarea
                  value={editContent}
                  onChange={(e) => onEditContentChange(e.target.value)}
                  className="w-full text-sm p-2 border border-gray-200 rounded resize-none focus:outline-none focus:ring-1 focus:ring-blue-400"
                  rows={3}
                  autoFocus
                />
                <div className="flex gap-1">
                  <button onClick={() => onSaveEdit(note.id)} className="text-[11px] px-2 py-0.5 bg-blue-500 text-white rounded hover:bg-blue-600">Save</button>
                  <button onClick={onCancelEdit} className="text-[11px] px-2 py-0.5 bg-gray-100 text-gray-600 rounded hover:bg-gray-200">Cancel</button>
                </div>
              </div>
            ) : (
              <>
                ...
                <div className="flex flex-wrap items-center gap-1 mt-1">
                  {noteTags.map((tag) => (
                    <span key={tag} className="text-[10px] px-1.5 py-0.5 bg-blue-50 text-blue-600 rounded-full flex items-center gap-0.5">
                      {tag}
                      {editingTagsNoteId === note.id && (
                        <button onClick={() => onRemoveTag(note.id, tag)} className="hover:text-red-500">&times;</button>
                      )}
                    </span>
                  ))}
                </div>
                ...
                {editingTagsNoteId === note.id && (
                  <div className="flex items-center gap-1 mt-1">
                    <input
                      ...
                      placeholder="Add tag..."
                      className="text-xs px-2 py-0.5 border border-gray-200 rounded w-24 focus:outline-none focus:ring-1 focus:ring-blue-400"
                      autoFocus
                    />
                    <button onClick={() => onAddTag(note.id)} className="text-[11px] px-1.5 py-0.5 bg-blue-500 text-white rounded hover:bg-blue-600">Add</button>
                  </div>
                )}
                {/* Note actions */}
                <div className="flex items-center gap-1 mt-1">
                  <button onClick={() => onStartEdit(note)} className="text-[10px] text-gray-400 hover:text-blue-500">Edit</button>
                  <button onClick={() => onToggleTagsEdit(note.id)} className="text-[10px] text-gray-400 hover:text-blue-500">Tags</button>
                  <button onClick={() => onDeleteNote(note.id)} className="text-[10px] text-gray-400 hover:text-red-500 ml-auto">Delete</button>
                </div>
              </>
            )}
          </div>
      ...
      {/* Add note form */}
      {addingNote && (
        <div className="ml-3 mt-2 pl-3 border-l-2 border-blue-200 space-y-1">
          <textarea
          ...
          />
          <div className="flex gap-1">
            <button onClick={onSaveNewNote} className="text-[11px] px-2 py-0.5 bg-blue-500 text-white rounded hover:bg-blue-600">Save</button>
            <button onClick={onCancelNewNote} className="text-[11px] px-2 py-0.5 bg-gray-100 text-gray-600 rounded hover:bg-gray-200">Cancel</button>
          </div>
        </div>
      )}
    </div>
